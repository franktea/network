		...
		::google::protobuf::Closure* done)
{
	...
	length = htonl(length);
	binary_req_pkg.insert(0, std::string((const char*)(&length), sizeof(length)));
	...
	socket_.receive(asio::buffer(&binary_resp_pkg[0], binary_resp_pkg.length()));
...
void RpcServer::RegisterService(::google::protobuf::Service* service)
{
    ...
    {
        item.method_descs.insert(std::make_pair(
                item.service_desc->method(i)->name(),
        ...
        std::shared_ptr<Session> session)
{
    ...
    ::google::protobuf::Message* request = item.service->GetRequestPrototype(pmd).New();
    request->ParseFromString(pkg_request->pkg());
    ::google::protobuf::Message* response = item.service->GetResponsePrototype(pmd).New();
...
void RpcServer::OnResponse(std::pair<::google::protobuf::Message*, ::google::protobuf::Message*> msgs,
