...
public:
    ...
    virtual std::string ErrorText() const override { return ""; };
...
public:
    SyncChannel(asio::io_context& ioc):socket_(ioc){}
    void Init(const std::string& ip, int port)
    {
        socket_.connect(tcp::endpoint(asio::ip::address::from_string(ip), port));
    }
...
class RpcServer
{
    ...
    public:
        ...
        void Response(::google::protobuf::Message* response, std::shared_ptr<RpcPackage> rpc_pkg)
        {
            auto p = shared_from_this();
            ...
            length = htonl(length);
            buffer_.insert(0, std::string((const char*)(&length), sizeof(length)));

            asio::async_write(socket_, asio::buffer(buffer_), [p, this](const asio::error_code& err, size_t len){
                if(err) {
...
public:
    RpcServer(const std::string& ip, int port):
        ep_(address::from_string(ip), port),
        acceptor_(ioc_, ep_){}
...
private:
    ...
    void OnResponse(std::pair<::google::protobuf::Message*, ::google::protobuf::Message*> msgs,
