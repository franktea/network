...
{
public:
    explicit Sock5Session(asio::io_context& ioc) :
            io_context_(ioc), local_socket_(ioc), dest_socket_(ioc), resolver_(
                    ioc)
    {
    }
public:
    void Start()
    {
        local_socket_.async_receive(asio::buffer(local_buffer_),
                std::bind(&Sock5Session::HandleLocalConnect, shared_from_this(),
                        _1, _2));
    }
...
private:
    void HandleLocalConnect(const asio::error_code& err,
            std::size_t bytes_transferred) // recv 05 01 00 from client
    {
        if (err)
        {
        ...
        {
            cout << "recv 05 01 00 not enough, bytes_transferred = "
                    << bytes_transferred << "\n";
            return;
        }

        if (local_buffer_[0] != 0x05 || local_buffer_[1] != 0x01
                || local_buffer_[2] != 0x00)
        {
            cout << "recv 05 01 00 content error " << int(local_buffer_[0])
                    << ", " << int(local_buffer_[1]) << ", "
                    << int(local_buffer_[1]) << "\n";
            return;
        }
        ...
        asio::async_write(local_socket_, asio::buffer(&local_buffer_[0], 2),
                std::bind(&Sock5Session::HandleConnectWrite, shared_from_this(),
                        _1, _2));
    }

    void HandleConnectWrite(const asio::error_code& err,
            std::size_t bytes_transferred) // after send 05 00 to client
    {
        if (err)
        {
        ...
        local_socket_.async_receive(asio::buffer(local_buffer_),
                std::bind(&Sock5Session::HandleConnectCmd, shared_from_this(),
                        _1, _2));
    }

    void HandleConnectCmd(const asio::error_code& err,
            std::size_t bytes_transferred)
    {
        ...
        {
            cout << "recv 05 01 00 01/03 not enough, bytes_transferred = "
                    << bytes_transferred << "\n";
            return;
        }
        ...
        if (local_buffer_[1] == 0x01) // connect as ipv4 address
        {
            if (local_buffer_[3] == 0x01) // 05 01 00 01 ip[4] port[2]
            {
                ...
                {
                    cout << "connect with ip length error, not 10, but "
                            << bytes_transferred << "\n";
                    return;
                }
                ...
                cout << "connect with ip[4]=" << dest_host_ << ", port="
                        << dest_port_ << "\n";
            }
            else if (local_buffer_[3] == 0x03) // 05 01 00 03 host_len host[host_len] port[2]
            {
                ...
                {
                    cout << "connect with hostname length error, not "
                            << 7 + host_len << ", but " << bytes_transferred
                            << "\n";
                    return;
                }
                ...
                uint16_t int_port =
                        *(uint16_t*) (&local_buffer_[bytes_transferred - 2]);
                dest_port_ = std::to_string(ntohs(int_port));

                cout << "connect with host=" << dest_host_ << ", port="
                        << dest_port_ << "\n";
            }
    ...
    {
        auto self = shared_from_this();
        local_socket_.async_read_some(asio::buffer(local_buffer_),
                [self, this](const asio::error_code& err, size_t bytes_transferred)
                {
                    if(! err)
                    {
    ...
    {
        auto self = shared_from_this();
        dest_socket_.async_read_some(asio::buffer(dest_buffer_),
                [self, this](const asio::error_code& err, size_t bytes_transferred)
                {
                    if(! err)
                    {
...
{
public:
    explicit Sock5Server(asio::io_context& ioc,
...
public:
    void Start()
    {
        ...
        acceptor_.async_accept(p->LocalSocket(),
