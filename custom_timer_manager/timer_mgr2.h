...
template<>
class timer_queue<
        detail::chrono_time_traits<asio::chrono::steady_clock,
                asio::wait_traits<asio::chrono::steady_clock>>> : public timer_queue_base
{
public:
    ...
    virtual long wait_duration_msec(long max_duration) const override
    {
        ...
        return this->to_msec(
                Time_Traits::to_posix_duration(
                        Time_Traits::subtract(timers_.begin()->first,
                                Time_Traits::now())), max_duration);
    }
            ...
            wait_op* op)
    {
        ...
        std::cout << "insert a timer which will fire after "
