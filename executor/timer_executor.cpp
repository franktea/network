...
asio::awaitable<void> Print()
{
    std::cout<<"hello in coro\n";
    co_return;
}
...
int main()
{
    ...
    auto e4 = scheduler.get_executor(std::chrono::steady_clock::now() + std::chrono::milliseconds(4000));
    asio::dispatch(e1, [](){ std::cout<<"hello after 1s\n"; });
    asio::dispatch(e2, [](){ std::cout<<"hello after 2s\n"; });
    asio::post(e3, [](){ std::cout<<"hello after 3s\n"; });
    asio::post(e4, [&scheduler](){ scheduler.stop(); std::cout<<"stopped after 4s\n"; }); // 4s后停止
    asio::co_spawn(scheduler, Print(), asio::detached);
    scheduler.run();
}
